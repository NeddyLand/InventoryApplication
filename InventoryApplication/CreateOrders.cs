...
namespace InventoryApplication
{
    public partial class CreateOrders : Form
    {
        ...
        private void loadCustomerDataFromDB()
        {
            ...
            {
                String query = "SELECT CustomerID as 'ID', FirstName as 'Имя', LastName as 'Фамилия', Phone as 'Телефон', Email, City as 'Город', Address as 'Адрес', Balance FROM Customers";
                using (SqlCommand sqlCommand = new SqlCommand(query, connection))
                {
                    ...
                    {
                        MessageBox.Show("ERROR while loading Customers from database");
                    }
                    finally
                    {
                        connection.Close();
        ...
        private void loadProductsFromDB()
        {
            using (SqlConnection connection = new SqlConnection(Properties.Settings.Default.connString))
            {
                String query = "SELECT ProductID as 'ID', ProductName as 'Наименование', ProductPrice as 'Цена', ProductQuantity as 'Кол-во', CategoryName as 'Товарная группа' FROM Products";
                using (SqlCommand sqlCommand = new SqlCommand(query, connection))
                {
                    try
                    {
                    ...
                    {
                        MessageBox.Show("ERROR while loading Products from database");
                    }
                    finally
                    {
                        connection.Close();
        ...
        {
            productsDT.DefaultView.RowFilter = string.Format("Наименование LIKE '%{0}%'", txtProductSearch.Text);
        }
        ...
        private void btnCreateOrder_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Убедитесь что выбран клиент и добавлен товар", "Ошибка");
            }
        ...
        private void insertIntoSalesTable()
        {
            using (SqlConnection connection = new SqlConnection(Properties.Settings.Default.connString))
            {
                string query = "INSERT INTO sales(OrderID, FirstName, ProductID, Quantity, TotalPrice) VALUES(@OrderID, @FirstName, @ProductID, @Quantity, @TotalPrice)";
                for (int i=0; i < dataGridOrder.Rows.Count; i++)
                {
                    ...
                    sqlCommand.Parameters.AddWithValue("@ProductID", dataGridOrder.Rows[i].Cells[0].Value);
        ...
        private void updateProductsAfterOrderCreation()
        {
            ...
            using (SqlConnection connection = new SqlConnection(Properties.Settings.Default.connString))
            {
                ...
                MessageBox.Show("Новый заказ успешно создан", "Инфо");
            }
