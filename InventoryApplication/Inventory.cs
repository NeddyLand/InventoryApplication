...
namespace InventoryApplication
{
    public partial class Inventory : Form
    {
                ...
                using (SqlConnection connection = new SqlConnection(Properties.Settings.Default.connString))
                {
                    ...
                    using (SqlCommand sqlCommand = new SqlCommand(query, connection))
                    {
                        try
                        {
                            ...
                            productsDataTable.Clear();
                            loadProductsFromDB();
                            MessageBox.Show("Новый товар успешно добавлен", "Инфо");
                        }
                        catch
                        {
                ...
                using (SqlConnection connection = new SqlConnection(Properties.Settings.Default.connString))
                {
                    ...
                    using (SqlCommand sqlCommand = new SqlCommand(query, connection))
                    {
                        try
                        {
                            ...
                            productsDataTable.Clear();
                            loadProductsFromDB();
                            MessageBox.Show("Данные о товаре обновлены", "Инфо");
                        }
                        catch
                        {
        ...
        private void btnDeleteProduct_Click(object sender, EventArgs e)
        {
            if (validateProduct())
            {
                DialogResult dialogResult = MessageBox.Show("Вы уверены что хотите удалить выбранный товар?", "Внимание!", MessageBoxButtons.YesNo);
                    ...
                    using (SqlConnection connection = new SqlConnection(Properties.Settings.Default.connString))
                    {
                        ...
                        using (SqlCommand sqlCommand = new SqlCommand(query, connection))
                        {
                            try
                            {
                                ...
                                productsDataTable.Clear();
                                loadProductsFromDB();
                                MessageBox.Show("Товар успешно удален!", "Инфо");
                            }
                            catch
                            {
        ...
        {
            productsDataTable.DefaultView.RowFilter = string.Format("Наименование LIKE '%{0}%'", txtProductSearch.Text);
        }
